...
// aplica modificadores de Debilidade
attrPairs.forEach(({ val, mod, deb }) => {
    const modEl = document.getElementById(mod);
    const valEl = document.getElementById(val);
    const debEl = document.getElementById(deb);
    const checkbox = debEl.addEventListener('click', (e) => {

        if ((modEl.value && valEl.value) || (!modEl.value && !valEl.value && !debEl.checked)) {
            if (debEl.checked) {
                modEl.value = parseInt(modEl.value) - 1;
                modEl.className = 'deb-Modifier-active';
            } else {
                modEl.value = parseInt(modEl.value) + 1;
                modEl.classList.remove('deb-Modifier-active');
            }
        } else {
            e.preventDefault();
        }
...
// document.getElementById('spell:has(.blue_filling)').addEventListener('click', () => {
