...
class CharacterSheet {
    ...
    renderMovements(searchQuery = '') {
        ...
        this.renderMoves(movementList, 'basicMoves', this.movementListFormat);
    }

    renderMoves(movementList, container, searchQuery = '') {
        const containerEl = document.getElementById(container);

        if(containerEl){
            if (!movementList.length) {
                containerEl.innerHTML = '<div class="movement-list"><em>Não há movimentos disponíveis para esta classe.</em></div>';
            return;
            }

            containerEl.removeChild(containerEl.firstChild); // Clear previous content
            ...
            containerEl.appendChild(scroll_group);
        }
    }
